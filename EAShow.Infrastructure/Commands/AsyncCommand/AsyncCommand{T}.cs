...
using EAShow.Infrastructure.Abstractions.Interfaces;
using Nito.Mvvm;

namespace EAShow.Infrastructure.Commands.AsyncCommand
{
    /// <summary>
    /// A strongly typed parameterized asynchronous command, which forwards it's implementation to <see cref="EAShow.Infrastructure.Commands.AsyncCommand"/>.
    /// </summary>
    public sealed class AsyncCommand<T> : AsyncCommandBase, IAsyncCommand<T>, INotifyPropertyChanged
    {
        private readonly Nito.Mvvm.AsyncCommand _asyncCommand;
        ...
        {
            _asyncCommand = new Nito.Mvvm.AsyncCommand(async parameter => await executeAsync((T)parameter), canExecuteChangedFactory);
