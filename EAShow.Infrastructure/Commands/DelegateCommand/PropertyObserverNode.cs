...
namespace EAShow.Infrastructure.Commands.DelegateCommand
{
    ...
    internal class PropertyObserverNode
    {
        ...
        {
            var propertyInfo = _inpcObject.GetType().GetRuntimeProperty(name: PropertyName); // TODO: To cache, if the step consume significant performance. Note: The type of _inpcObject may become its base type or derived type.
            var nextProperty = propertyInfo.GetValue(obj: _inpcObject);
            ...
            Next.SubscribeListenerFor(inpcObject: nextInpcObject);
        }
